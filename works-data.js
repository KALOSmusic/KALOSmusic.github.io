/* ============================================================
   ARCHIVIO ALBUM KALOS
   ============================================================

   COME AGGIUNGERE UN NUOVO ALBUM
   --------------------------------
   1. Copia un intero blocco { ... } da qui sotto (da "{" alla
      "}," che lo chiude), incollalo appena PRIMA della riga
      "];" in fondo a questo file.
   2. Cambia i valori con quelli del nuovo album.
   3. Carica le immagini (copertina e galleria) nella STESSA
      cartella dove si trovano già index.html e gli altri file
      (la radice del repository — non esiste una sottocartella
      "assets") e scrivi qui i loro nomi esatti.
   4. Salva, carica questo file su GitHub sovrascrivendo quello
      vecchio.

   L'album che metti PIÙ IN ALTO nell'elenco qui sotto sarà il
   PRIMO a comparire nella pagina "Works".

   CAMPI DI OGNI ALBUM
   --------------------------------
   title    → Nome dell'album. Ricordati le virgolette attorno al
              titolo, es. "Lorna — \"Insatiable\"" oppure "\"TUESDAY\""
              (se l'album non ha un nome artista a parte).
   genre    → Genere musicale, es. "Funk / Rock / Afrobeat".
              Se l'album è strumentale (senza testi), scrivi
              "Instrumental" come primo elemento, es.
              "Instrumental / Noise / Post-Hardcore".
   cover    → Nome del file immagine di copertina (usato anche
              nell'elenco Works), es. "lorna-insatiable.jpg"
   instrumental → Scrivi "true" (senza virgolette) SOLO se l'album
              non ha testi da mostrare: in quel caso il pulsante
              "Lyrics" accanto a ogni brano non comparirà affatto.
              Per un album con testi, ometti del tutto questa riga
              (non serve scrivere "false").
   concept  → Il testo del Concept, in due lingue:
              concept.en → versione inglese
              concept.it → versione italiana
              Un pulsante EN/IT sul sito farà passare dall'una
              all'altra. Ogni paragrafo va tra <p> e </p>.
              Per il corsivo usa <em>parola</em>, per il
              grassetto <strong>parola</strong>.
   images   → Elenco delle immagini della galleria del Concept
              (scorrono una alla volta, con swipe). Nomi dei
              file, nello stesso ordine in cui vuoi che compaiano.
   youtubePlaylistId → Solo il codice della playlist YouTube,
              si trova nel link dopo "list=".
   tracks   → Elenco delle tracce, NELL'ORDINE REALE della
              playlist YouTube. Ogni traccia è un oggetto con:
              title  → nome della canzone
              lyrics → testo completo della canzone. Lascia ""
                       se non lo hai ancora (o ometti il campo del
                       tutto per un album "instrumental: true").
                       Per andare a capo, usa \n. Il testo compare
                       con un pulsante "Lyrics" accanto al brano,
                       dentro la stessa pagina Audio (non
                       interrompe la riproduzione).
   ============================================================ */

const KALOS_WORKS = [
  {
    title: "Morchia Quartet — \"Morchia 1\"",
    genre: "Instrumental / Noise / Post-Hardcore / Jazzcore",
    cover: "morchia-1-cover.jpg",
    instrumental: true,

    concept: {
      en: `
        <p>Morchia Quartet is a fictional four-piece band built around a simple idea: music without rules, without comfort, without the need to explain itself. The four members always appear with their faces covered by black balaclavas — no biography, no story about where they come from, no explanation of who they really are. They exist only through their music.</p>
        <p>The lineup is drums, guitar, bass and saxophone. The saxophonist moves freely between soprano, alto, tenor and baritone, using the instrument as much as a source of noise and physical energy as a melodic voice — screaming, scraping, colliding with the guitars, or dissolving into the rhythm section like another piece of distorted machinery.</p>
        <p>Morchia Quartet is rooted in experimental music and in the more extreme, unconventional sides of rock, jazz and contemporary sound — drawing from early-1990s noise and post-hardcore, jazzcore, math-influenced rhythms and free-form experimentation, treated as raw material and recombined into something deliberately unstable.</p>
        <p>The music can be brutal and abrasive, then suddenly intricate and unexpectedly fluid: heavy riffs collide with fragmented rhythms, chaotic passages give way to strange melodic shapes, structure dissolves into noise and improvisation — never as technical display, but to build tension and keep the listener unsure of where the music is going.</p>
        <p>The result is a collection of short, intense pieces that refuse to settle into predictable forms — some direct and violent, others abstract and atmospheric, all sharing the same restlessness. This is not music meant to sit comfortably in the background: physical, claustrophobic, disorienting, yet strangely hypnotic.</p>
        <p>Their first album, Morchia 1, consists of 37 instrumental fragments — 37 small explosions with no conventional narrative or song structure, always avoiding the obvious solution.</p>
        <p>Morchia Quartet is, above all, an exercise in musical freedom: four anonymous figures, four instruments, and an open space in which almost anything can happen.</p>
      `,
      it: `
        <p>Morchia Quartet è una band immaginaria a quattro elementi, costruita attorno a un'idea semplice: musica senza regole, senza comodità, senza bisogno di spiegarsi. I quattro membri compaiono sempre con il volto coperto da passamontagna neri — nessuna biografia, nessuna storia sulla loro provenienza, nessuna spiegazione su chi siano davvero. Esistono solo attraverso la musica.</p>
        <p>La formazione è batteria, chitarra, basso e sassofono. Il sassofonista si muove liberamente tra soprano, contralto, tenore e baritono, usando lo strumento tanto come fonte di rumore ed energia fisica quanto come voce melodica — urlando, graffiando, scontrandosi con le chitarre, o dissolvendosi nella sezione ritmica come un altro pezzo di macchinario distorto.</p>
        <p>Morchia Quartet affonda le radici nella musica sperimentale e nei lati più estremi e non convenzionali di rock, jazz e suono contemporaneo — attingendo dal noise e dal post-hardcore dei primi anni '90, dal jazzcore, da ritmiche math e dalla sperimentazione a briglia sciolta, trattati come materia prima e ricombinati in qualcosa di deliberatamente instabile.</p>
        <p>La musica può essere brutale e abrasiva, poi improvvisamente intricata e inaspettatamente fluida: riff pesanti si scontrano con ritmiche frammentate, passaggi caotici si aprono a forme melodiche strane, la struttura si dissolve in rumore e improvvisazione — mai come esercizio di stile, ma per creare tensione e tenere chi ascolta incerto su dove la musica stia andando.</p>
        <p>Il risultato è una raccolta di brani brevi e intensi, che rifiutano di adagiarsi in forme prevedibili — alcuni diretti e violenti, altri astratti e atmosferici, tutti animati dalla stessa inquietudine. Non è musica pensata per stare comoda sullo sfondo: fisica, claustrofobica, disorientante, eppure stranamente ipnotica.</p>
        <p>Il loro primo album, Morchia 1, è composto da 37 frammenti strumentali — 37 piccole esplosioni senza una narrazione o una struttura convenzionale, sempre alla ricerca di tutto tranne che della soluzione ovvia.</p>
        <p>Morchia Quartet è, prima di tutto, un esercizio di libertà musicale: quattro figure anonime, quattro strumenti, e uno spazio aperto in cui può succedere quasi tutto.</p>
      `
    },

    images: [
      "morchia-gallery-1.jpg",
      "morchia-gallery-2.jpg",
      "morchia-gallery-3.jpg",
      "morchia-gallery-4.jpg",
      "morchia-gallery-5.jpg",
      "morchia-gallery-6.jpg"
    ],

    youtubePlaylistId: "PLH3P1wMJ7vBg",

    tracks: [
      { title: "Morchia #1" },
      { title: "Morchia #2" },
      { title: "Morchia #5" },
      { title: "Morchia #8" },
      { title: "Morchia #9" },
      { title: "Morchia #10" },
      { title: "Morchia #17" },
      { title: "Morchia #19" },
      { title: "Morchia #22" },
      { title: "Morchia #23" },
      { title: "Morchia #25" },
      { title: "Morchia #27" },
      { title: "Morchia #28" },
      { title: "Morchia #29" },
      { title: "Morchia #30" },
      { title: "Morchia #31" },
      { title: "Morchia #32" },
      { title: "Morchia #33" },
      { title: "Morchia #34" },
      { title: "Morchia #35" },
      { title: "Morchia #36" },
      { title: "Morchia #37" },
      { title: "Morchia #38" },
      { title: "Morchia #39" },
      { title: "Morchia #40" },
      { title: "Morchia #41" },
      { title: "Morchia #43" },
      { title: "Morchia #45" },
      { title: "Morchia #46" },
      { title: "Morchia #47" },
      { title: "Morchia #51" },
      { title: "Morchia #53" },
      { title: "Morchia #54" },
      { title: "Morchia #55" },
      { title: "Morchia #56" },
      { title: "Morchia #57" },
      { title: "Morchia #58" }
    ]
  },
  {
    title: "Lorna — \"Insatiable\"",
    genre: "Funk / Rock / Afrobeat",
    cover: "lorna-insatiable.jpg",

    concept: {
      en: `
        <p>Lorna is a young Black singer, an instinctive, fierce, magnetic woman who has chosen to live by her own rules.</p>
        <p>She doesn't look for approval, she doesn't ask for permission, and most of all, she doesn't feel the need to explain her choices. Her freedom is not a slogan: it is how she lives every day, an almost animal-like attitude toward life.</p>
        <p>Lorna is sensual without being submissive, provocative without trying to shock, aggressive without turning her strength into an act. She knows exactly what she wants and, above all, she knows what she doesn't want. She belongs to no one.</p>
        <p>Her songs are about desire, sex, anger, independence, pleasure, rejection, and instinct. They talk about men, but rarely to win them over: most of the time, they are the ones who have to keep up with her. Lorna can desire someone without falling in love, seduce without promising anything, take what she wants, and walk away without giving any explanations.</p>
        <p>Musically, her world comes from a wild mix of Funk, Rock, and Afrobeat: physical, obsessive grooves, pounding bass, dirty guitars, percussive rhythms, and sudden bursts of energy. The music should feel physical, sweaty, and primal. It is not made to be elegant or comforting: it is made to move the body.</p>
        <p>Lorna sings as if every song were a form of liberation. Her voice can be sensual, rough, shouted, sarcastic, or almost animal-like; she can go from seduction to threat without asking for permission.</p>
        <p>Her sexuality is not decoration: it is power. Her anger is not self-pity: it is energy. Her independence is not a statement: it is simply the way she exists.</p>
      `,
      it: `
        <p>Lorna è una giovane cantante afroamericana, una donna istintiva, feroce e magnetica che ha scelto di vivere secondo le proprie regole.</p>
        <p>Non cerca approvazione, non chiede permesso e soprattutto non sente il bisogno di giustificare le proprie scelte. La sua libertà non è uno slogan: è un comportamento quotidiano, un atteggiamento quasi animalesco verso la vita.</p>
        <p>Lorna è sensuale senza essere accomodante, provocatoria senza cercare di scandalizzare, aggressiva senza trasformare la propria forza in una posa. Sa esattamente cosa vuole e, soprattutto, sa cosa non vuole. Non appartiene a nessuno.</p>
        <p>Le sue canzoni raccontano desiderio, sesso, rabbia, indipendenza, piacere, rifiuto e istinto. Parlano di uomini, ma raramente per conquistarli: spesso sono loro a dover stare al passo con lei. Lorna può desiderare qualcuno senza innamorarsi, sedurre senza promettere nulla, prendere ciò che vuole e andarsene senza lasciare spiegazioni.</p>
        <p>Musicalmente, il suo universo nasce dall'incontro selvaggio tra Funk, Rock e Afrobeat: groove fisici e ossessivi, bassi pulsanti, chitarre sporche, ritmi percussivi e improvvise esplosioni di energia. La musica deve avere qualcosa di corporeo, sudato e primordiale. Non è costruita per essere elegante o rassicurante: è costruita per muovere il corpo.</p>
        <p>Lorna canta come se ogni brano fosse una forma di liberazione. La sua voce può essere sensuale, roca, urlata, sarcastica o quasi animalesca; passa dalla seduzione alla minaccia senza chiedere il permesso.</p>
        <p>Il suo erotismo non è decorativo: è potere. La sua rabbia non è vittimismo: è energia. La sua indipendenza non è una dichiarazione: è il suo modo naturale di esistere.</p>
      `
    },

    images: [
      "lorna-gallery-1.jpg",
      "lorna-gallery-2.jpg",
      "lorna-gallery-3.jpg",
      "lorna-gallery-4.jpg",
      "lorna-gallery-5.jpg",
      "lorna-gallery-6.jpg"
    ],

    youtubePlaylistId: "PLOpjGYfNpjeY",

    tracks: [
      {
        title: "Still",
        lyrics: "I had it once, I want it twice,\nthe same heat at a different price.\nIt never left, it never cools,\nit pulls me back and breaks my rules.\n\nI know the weight, I know the taste,\nI want it slow, I want it chased.\nNot just the end, not just the peak,\nthe whole damn climb is what I seek.\n\nMy skin remembers every touch,\nand now it says it needs as much.\nIt calls the heat back to the bone,\nmy body will not leave it alone.\n\nI want the hands, I want the press,\nthe give, the pull, the sweet excess.\nThe breath that breaks, the grip that stays,\nI want it back, I want it days.\n\nNot done, not close, not satisfied,\nthe want comes back, it never died.\nAgain the weight, again the heat,\nagain until I lose my feet.\n\nI know the weight, I know the taste,\nI want it slow, I want it chased.\nNot just the end, not just the peak,\nthe whole damn climb is what I seek\n\nNot done, not close, not satisfied,\nthe want comes back, it never died.\nAgain the weight, again the heat,\nagain until I lose my feet.\n\nNot done, not close, not satisfied,\nthe want comes back, it never died.\nAgain the weight, again the heat,\nagain until I lose my feet."
      },
      {
        title: "Bored",
        lyrics: "bored...\nbored...\n\nAnother face, another night,\nyou talk and talk but nothing's right.\nNo spark behind those pretty eyes,\njust rehearsed moves and second-hand lies.\n\nbored...\n\nYou wear your shine like borrowed gold,\nI've touched hotter, harder, bold.\nYou got the body, not the fire,\njust enough to feed desire.\n\nbored...\n\nI'll take what's good and leave the rest,\nyou passed the skin but failed the test.\nOne night is all you'll ever be,\ntoo small a world to carry me.\n\nbored...\n\nSo come on then, just play your part,\nbored...\nI want the flesh, not what you call your heart.\nbored...\nGive me the heat, then let it end,\nbored...\nyou're not a lover, not a friend.\nbored...\n\nJust passing skin inside the dark,\nanother shape without a spark.\nI've seen a hundred just like you —\nenough to use, not enough to keep.\n\nSo come on then, just play your part,\nI want the flesh, not what you call your heart.\nGive me the heat, then let it end,\nyou're not a lover, not a friend.\n\nbored...\n\nJust passing skin inside the dark,\nanother shape without a spark.\nI've seen a hundred just like you —\nenough to use, not enough to keep.\n\nBored\nBored\nI'm\nso\nBored"
      },
      {
        title: "Hunt",
        lyrics: "I saw him from across the room,\nthe way he moved cut through the gloom.\nI didn't think, I didn't wait,\nI walked toward him, sealed his fate.\n\nI touched his arm, I held his eyes,\nno question asked, no sweet disguise.\nI know what I want when I see it clear,\nI pulled him close, I made it near.\n\nHe said my name like it was new,\nI said nothing, I just knew.\nMy hands already knew the place,\nmy mouth already knew his face.\n\nI take what calls me, take it straight,\nI don't ask twice, I don't debate.\nThe body speaks before the mind,\nI leave the hesitation behind.\n\nI had him warm, I had him real,\nI took my time, I took my feel.\nNot his to give, not mine to keep,\njust something good and dark and deep.\n\nI walked away while he still burned,\nthat's all I wanted, all I earned.\nThe hunt is over, hunger fed —\nuntil the next one turns his head.\n\nI take what calls me, take it straight,\nI don't ask twice, I don't debate.\nThe body speaks before the mind,\nI leave the hesitation behind."
      },
      {
        title: "Again",
        lyrics: "I touch my skin, I know my heat,\nbut it is never quite complete.\nIt rises fast, it falls too soon,\nlike broken light inside a room.\n\nI want it more, I want it now,\nI feel it pull me, teach me how.\nNo quiet edge, no fading spark,\nI need it loud, I need it sharp.\n\nI call the want, I call the fire,\nit twists and turns into desire.\nIt shakes my breath, it bends my spine,\nbut still it is not fully mine.\n\nI need a body not my own,\nto take the space I've never known.\nTo match the pulse I cannot slow,\nto make it rise, to make it grow.\n\nI want it hard, I want it real,\nnot just the way I make it feel.\nAgain, again, until it breaks,\nuntil it takes more than it takes.\n\nI want it hard, I want it real,\nnot just the way I make it feel.\nAgain, again, until it breaks,\nuntil it takes more than it takes.\n\nI want it hard, I want it real,\nnot just the way I make it feel.\nAgain, again, until it breaks,\nuntil it takes more than it takes."
      },
      {
        title: "Insatiable",
        lyrics: "My skin is burning, white and hot\nI'm craving things that you are not\nMy chest is tight, my breath is thick\nI need a thrill, I need it quick.\n\nMy peaks are rising, hard and cold\nI'm acting loose, I'm feeling bold\nDown in the dark, I'm slick and wide\nI'm aching for a place to hide.\n\nHard as a rock, my back is arched\nThe fire grows, my soul is parched\nMy hips are heavy, round and tight\nReady to ride you through the night.\n\nI feel the throb, I feel the sting\nI want the sweat that hunger brings\nMuscles twitching, raw and lean\nThe dirtiest thing you've ever seen.\n\nMy pulse is screaming through the floor\nI taste the salt and want some more\nMy body's locked in a fever dream\nI want to hear the silence scream.\n\nDon't ask me why, don't ask me when\nI'll take you down and back again\nI'm never full, I'm never still\nI live to hunt, I live to thrill.\n\nHard as a rock, my back is arched\nThe fire grows, my soul is parched\nMy hips are heavy, round and tight\nReady to ride you through the night.\n\nMy peaks are rising, hard and cold\nI'm acting loose, I'm feeling bold\nDown in the dark, I'm slick and wide\nI'm aching for a place to hide.\n\nDon't ask me why, don't ask me when\nI'll take you down and back again\nI'm never full, I'm never still\nI live to hunt, I live to thrill.\n\nDon't ask me why, don't ask me when\nI'll take you down and back again\nI'm never full, I'm never still\nI live to hunt, I live to thrill."
      },
      {
        title: "Don't Love Me",
        lyrics: "Don't love me, keep that lie,\nI came for heat, then goodbye.\nDon't bring me dreams, don't bring me chains,\nI want your fire, not your name.\n\nDon't whisper futures in my bed,\nI want your hands, not what you said.\nNo candle vows, no soft belief,\nI take my pleasure clean and brief.\n\nI fuck you once, then you're out,\nthat's what this night is all about.\nNo tears, no keys, no second plea,\nyou got your turn, now let me leave.\n\nI hate the eyes that ask for more,\nthe kind that wait outside my door.\nYou had the dark, you had the spark,\nnow take your shadow, leave the mark.\n\nDon't love me, you'll regret\nwanting what you'll never get.\nTouch me right, then disappear—\nI was the storm, not something dear.\n\nI fuck you once, then you're out,\nthat's what this night is all about.\nNo tears, no keys, no second plea,\nyou got your turn, now let me leave.\n\nDon't love me, you'll regret\nwanting what you'll never get.\nTouch me right, then disappear—\nI was the storm, not something dear.\n\nI fuck you once, then you're out,\nthat's what this night is all about.\nNo tears, no keys, no second plea,\nyou got your turn, now let me leave."
      },
      {
        title: "No Permission",
        lyrics: "I walk where I want, I breathe what I take,\nno line on the ground I refuse to break.\nMy skin is mine, my heat is loud,\nI move through the world like I own this crowd.\n\nHands in the air, sweat on skin,\nI don't ask twice, I just go in.\nNo judge, no chain, no silent law,\nI don't live like I did before.\n\nNo permission, no apology,\nthis body moves in full liberty.\nNo permission, no one to obey,\nI take the night, I burn the day.\n\nWe drink too fast, we laugh too hard,\nlose all names in a broken bar.\nSmoke in lungs, fire in chest,\ntonight we live like we're possessed.\n\nI don't care what they call right,\nI follow noise, I follow light.\nIf it feels real, it stays with me,\nif it doesn't bleed, it's not for me.\n\nNo permission, no apology,\nthis body moves in full liberty.\nNo permission, no one to obey,\nI take the night, I burn the day.\n\nNo permission, no apology,\nthis body moves in full liberty.\nNo permission, no one to obey,\nI take the night, I burn the day.\n\nMove with me, don't think, just fall,\nno rules survive when the drums call.\nWe are the law, we are the sound,\nwe tear the silence to the ground.\n\nNo permission, no apology,\nthis body moves in full liberty.\nNo permission, no one to obey,\nI take the night, I burn the day.\n\nNo permission, no apology,\nthis body moves in full liberty.\nNo permission, no one to obey,\nI take the night, I burn the day.\n\nIf it feels real, it stays with me,\nif it doesn't bleed, it's not for me.\nIf it feels real, it stays with me,\nif it doesn't bleed, it's not for me.\nIf it feels real, it stays with me,\nif it doesn't bleed, it's not for me."
      },
      {
        title: "Take",
        lyrics: "I don't wait for doors to part,\nI move on hunger, move on heart.\nI see the thing, I feel the flame,\nI reach before it gets a name.\n\nNo crown was laid upon my head,\nno path was warmed, no table set.\nI came through teeth, I came through bone,\nI built my fire with hands alone.\n\nMan or woman, silk or dust,\nall of us are blood and rust.\nRich or ruined, praised or blamed,\nwe break the same, we burn the same.\n\nI keep no score of race or creed,\nI only measure want and need.\nWho fights may rise, who dares may win,\nthe gate is locked from deep within.\n\nDon't call me brave, don't call me rare,\nI took the road because it's there.\nI never asked what I was worth,\nI learned it walking on this earth.\n\nNo permission, no applause,\nI never lived inside your laws.\nI don't ascend because you crawl,\nI stand because I stood through all.\n\nThis is no flag for you to raise,\nno borrowed rage, no righteous praise.\nThis is the rhythm in my chest—\nI take what calls me, keep the rest.\n\nI am a woman, fierce and whole,\nno hand was needed for my soul.\nIf I desire, I move, I make—\nthat is my name.\nI take.\n\nI am a woman, fierce and whole,\nno hand was needed for my soul.\nIf I desire, I move, I make—\nthat is my name.\nI take.\n\nI am a woman, fierce and whole,\nno hand was needed for my soul.\nIf I desire, I move, I make—\nthat is my name.\nI take."
      },
      {
        title: "Get Gone",
        lyrics: "Who the fuck is in my bed,\nwhose arm is this, whose heavy head?\nLight hits hard, my skull feels dead,\nI was drunk off my fucking head.\n\nDon't look at me like this means shit,\nlast night burned out — that's it.\nYou stayed too long, you missed the cue,\nmorning's here and I'm done with you.\n\nGet up.\nGet dressed.\nGet gone.\nGet up.\nGet dressed.\nGet gone.\n\nGet your clothes up off my chair,\nyou don't belong in my air.\nYour breathing loud, your body heat,\nget your hands off my fucking sheets.\n\nYou smell like last night's bad idea,\nwarm beer, smoke and cheap veneer.\nDon't try to smile, don't try to speak,\nI want you gone before I freak.\n\nGet up.\nGet dressed.\nGet gone.\nGet up.\nGet dressed.\nGet gone.\n\nI don't do coffee, names or stays,\nI don't replay these kinds of days.\nWhatever this was died with dark,\nnow you're just noise after the spark.\n\nDoor's right there — use your feet,\nback to whatever owns your street.\nNo goodbye kiss, no second round,\njust get the fuck out of my house.\n\nGet up.\nGet dressed.\nGet the fuck out.\nGet up.\nGet dressed.\nGet the fuck out!\nGET THE FUCK OUT!"
      },
      {
        title: "Funk A",
        lyrics: "Bassline deep in your blood.\nSweat running down your back.\nToo hot.\nToo loud.\nToo close.\nFUNK.\nFUNK!\n\nToo hot.\nToo loud.\nToo close.\nFUNK!\n\nI GOT IT IN MY BLOOD!\nFUNK!\nYOU GOT IT IN YOUR BONES!\nFUNK!\nFUNK!\n\nMove.\nI said move.\nFUNK!\n\nDrums hit hard — bodies shake,\nsmoke in the room and the floor might break\nFUNK!\nFUNK!\n\nMore.\nGive me more.\nFUNK!\n\nHands on skin.\nHeat in mouths.\nNo thinking now.\nFUNK.\nFUNK!\n\nToo hot.\nToo loud.\nToo close.\nFUNK!\n\nShake it loose.\nLose your head.\nFeel the rhythm in your chest.\nFUNK!\n\nMOVE YOUR ASS\nFUNK!\nC'MON BABY\nFUNK!\nMOVE YOUR ASS\nFUNK!\n\nToo fast.\nToo hard.\nDon't stop.\nFUNK!\nFUNK!\n\nMOVE YOUR ASS!\nFUNK!\nFUNK!\n\nFUNK\nBlood in the groove.\nFUNK\nSweat in the air.\nFUNK\nNobody clean anymore.\nFUNK\nMove.\nMOVE.\nFUNK!"
      }
    ]
  },

  {
    title: "\"TUESDAY\"",
    genre: "Avant-garde / Experimental Metal — Theatrical, Grotesque",
    cover: "tuesday-cover.jpg",

    concept: {
      en: `
        <p>TUESDAY is a concept album about a man's gradual detachment from reality: a journey into a mind slipping into delusion, through episodes, obsessions and beliefs that grow increasingly disconnected from the world around him. Among them is the certainty that the arrival of a new TUESDAY will finally put everything back in its place.</p>
        <p>At first, his world seems only slightly out of balance: objects appear to have a will of their own, absurd bureaucratic procedures, endless lists, and voices and characters begin to inhabit his mind. Track by track, however, the boundary between reality, memory, imagination and delusion becomes increasingly fragile. The promise of Tuesday turns into a certainty, then into a faith, until it becomes the only thing capable of giving his universe any sense of order.</p>
        <p>The music follows the same process. TUESDAY moves through very different genres and musical languages — alternative rock, avant-garde, jazz, metal, funk, reggae, punk-ska and experimental music — without trying to smooth them into something comfortable. The identity of each track can suddenly twist, accelerate, collapse or reassemble itself into something completely different. Irregular rhythms, sudden shifts in atmosphere, dissonance, unpredictable arrangements and extreme contrasts become part of the narrative.</p>
        <p>The voice is one of the project's main instruments: not simply a narrator, but a constantly changing character. Its register, tone, theatricality and singing style shift continuously, moving from intimacy to caricature, from solemnity to hysteria, from melody to chaos.</p>
        <p>The album also explores the possibilities of generative music and the relationship between composition, prompting and AI interpretation. The goal is not to use AI to create a "perfect" song, but to make it part of a controlled creative process — an unpredictable instrument capable of producing music that is intentionally unstable, absurd and sometimes even wrong.</p>
        <p>TUESDAY is therefore a story about madness told through music that gradually loses control of itself.</p>
        <p>And when the ending finally arrives, everything seems to be fine.</p>
        <p>Maybe too fine.</p>
      `,
      it: `
        <p>TUESDAY è un concept album che racconta il progressivo distacco dalla realtà di un uomo: un viaggio dentro una mente delirante, attraverso episodi, ossessioni e convinzioni sempre più lontane dalla realtà. Tra queste, la certezza che l'arrivo di un nuovo TUESDAY rimetterà finalmente ogni cosa al suo posto.</p>
        <p>All'inizio il suo mondo appare soltanto leggermente fuori asse: oggetti che sembrano avere una volontà propria, procedure burocratiche assurde, liste interminabili, voci e personaggi che abitano la sua mente. Brano dopo brano, però, il confine tra realtà, ricordo, immaginazione e delirio diventa sempre più fragile. La promessa di Tuesday si trasforma in una certezza, poi in una fede, fino a diventare l'unico elemento capace di dare un ordine al suo universo.</p>
        <p>Anche la musica segue questo processo. TUESDAY attraversa generi e linguaggi molto diversi — alternative rock, avant-garde, jazz, metal, funk, reggae, punk-ska e musica sperimentale — senza cercare una fusione rassicurante. Le identità dei brani possono improvvisamente deformarsi, accelerare, collassare o ricomporsi in una nuova forma. Ritmi irregolari, cambi di atmosfera, dissonanze, arrangiamenti imprevedibili e contrasti estremi diventano parte della narrazione.</p>
        <p>La voce è uno degli strumenti principali del progetto: non una semplice voce narrante, ma un vero personaggio mutevole. Il registro, il timbro, la teatralità e il modo di cantare cambiano continuamente, passando dall'intimità alla caricatura, dalla solennità all'isteria, dalla melodia al caos.</p>
        <p>L'album nasce anche da una ricerca sulle possibilità della musica generativa e sul rapporto tra composizione, prompting e interpretazione artificiale. L'obiettivo non è utilizzare l'intelligenza artificiale per ottenere una canzone "perfetta", ma usarla come uno strumento imprevedibile all'interno di un processo creativo controllato: costruire intenzionalmente musica capace di comportarsi in modo instabile, assurdo e talvolta persino sbagliato.</p>
        <p>TUESDAY è quindi una storia sulla follia raccontata attraverso una musica che, progressivamente, perde anch'essa il controllo.</p>
        <p>E quando finalmente arriva l'epilogo, tutto sembra andare bene.</p>
        <p>Forse troppo bene.</p>
      `
    },

    images: [
      "tuesday-gallery-1.jpg",
      "tuesday-gallery-2.jpg",
      "tuesday-gallery-3.jpg"
    ],

    youtubePlaylistId: "PLft_DLJDxsDI",

    tracks: [
      {
        title: "End Of Tuesday",
        lyrics: "Good morning.\nThe hallway in Sector Seven has submitted its resignation.\nThe escalators refuse to comment.\nThank you.\n\nThe Department of Teaspoons has postponed sunrise until Tuesday.\nReason:\nExcessive Wednesday.\n\nAn official refrigerator is reviewing a map of water.\nIt nods.\nIt takes notes.\nIt congratulates a parenthesis.\n\nAttention.\nAttention.\n\nThe central ceiling fan has learned its legal name.\n(The legal name?)\nRepeat:\nThe central ceiling fan has learned its legal name.\n(That seems unnecessary.)\n\nThree chairs are holding a confidential meeting.\nOne claims to be an aqueduct.\n(This is not the first time.)\nThe other two, for reasons of professionalism, offer no objections.\n\nPlease remain seated.\nPlease remain standing.\nPlease remain approximately geometric.\n\nI am not the doorbell.\nI am the temporary replacement for the replacement of the replacement of the doorbell.\n\nYour patience has been upgraded.\nYour waiting privileges have been extended.\nThank you for waiting.\nThank you for continuing to wait.\nThank you for waiting incorrectly.\n\nForm 19-B has escaped the filing cabinet.\nForm 19-B is considered decorative.\n(It was always decorative.)\nForm 19-B should not be encouraged.\n\nA committee of shoelaces has approved the color of thunder.\nThe vote was unanimous.\nExcept for a cucumber.\n(Here we go again.)\nThe cucumber filed a complaint.\nThe complaint was promoted.\nThe promotion was denied.\nThe denial received an award.\n\nAttention.\nThe moon is now a temporary staircase.\n(Temporary?)\nPlease use alternate gravity.\n(Alternate gravity approved.)\nPlease do not fold the horizon.\nPlease do not teach mathematics to the smoke detector.\n\nThe Department of Teaspoons has reached a final decision.\nSunrise will proceed as scheduled.\nWith reservations.\nAnd two dissenting spoons.\n\nCongratulations.\nCongratulations.\nCongratulations on the weather.\nThe weather declined the offer.\n\nThank you for your cooperation.\nThank you for your cooperation.\nThank you for your coopera\u2014\n(There was no cooperation.)\nCorrection.\nThere was no cooperation.\n(We noticed.)\n\nEnd of report.\nEnd of hallway.\nEnd of cucumber.\n(The cucumber was never consulted.)\nEnd of Tuesday."
      },
      {
        title: "The Second List",
        lyrics: "I made a list.\nI lost the list.\nI made another list to find the first list.\n\nThe second list is also gone.\nI'm making a third list of things I need to remember about the lists.\n\nDid I eat today?\nI remember thinking about eating.\nI remember deciding to eat later.\nLater was three hours ago.\n\nI am currently eating a spoon.\nThe spoon is empty.\nI'm not sure when that happened.\n\nI wrote my name on a Post-it note so I wouldn't forget it.\nI forgot where I put the Post-it note.\nMy name feels provisional.\n\nThere are seventeen tabs open in my brain.\nOne of them is playing music I don't remember starting.\nOne of them is an argument from 2014 that I almost won.\nOne of them is just the word \"Tuesday\" with no further context.\nI cannot close any of them.\n\nI called someone.\nI forgot who.\nThey answered.\nI forgot why I called.\nWe stayed on the line for forty seconds in mutual silence.\nIt felt like an agreement.\nI hung up.\nThey probably understand.\n\nI went to the kitchen for something important.\nThe kitchen had no information.\nI went back to the other room.\nThe other room didn't remember either.\n\nI stood in the hallway as a neutral party.\nThe hallway suggested I sit down.\nI sat down in a different room.\nI don't know this room.\n\nI have three unfinished cups of coffee at different stages of cooling.\nThey represent my ambitions.\n\nSomeone sent me a message.\nIt said: \"Are you okay?\"\nI replied: \"Yes, absolutely, doing great.\"\nI have not moved in forty minutes.\nThe chair and I have reached an understanding.\n\nI should probably throw out the trash.\nI thought about throwing out the trash.\nI thought about thinking about throwing out the trash.\nI made a note to think about it later.\nThe note is on the second list.\n\nI need to lie down for a brief geological era.\nI'll be fine.\nI'll be fine.\nI'll be...ehm...\nWhat?!"
      },
      {
        title: "The Night Has Questions",
        lyrics: "I turned off the lamp about an hour ago.\nThe room is still here.\nThe ceiling is still here.\nThe blanket has accepted me as a temporary citizen.\n\nOutside, something is moving through the trees.\nOr maybe through my memory.\nIt's difficult to tell at this hour.\n\nI close my eyes.\nThe darkness arrives politely.\nIt takes a seat.\nIt waits.\n\nI remember a conversation from twelve years ago.\nNothing important.\nNobody died.\nNobody cried.\nBut somehow I am still editing my answers.\n\nThe refrigerator hums in another room.\nFor a moment, it sounds almost human.\n\nI think about tomorrow.\nThen next week.\nThen a mistake I made when I was seventeen.\nThen a song.\nThen an email.\nThen the fact that I never sent the email.\nThen why I never sent it.\nThen whether I should send it now.\n\nThe darkness is becoming crowded.\nSomeone is counting failures.\nSomeone is rearranging old embarrassments.\nSomeone is asking questions that have no practical application.\n\nDo penguins have knees?\nWhy did she laugh?\nWhere are my keys?\nWhat was I trying to remember?\n\nThe room is full now.\nEvery chair is occupied.\nEvery thought brought a friend.\nNobody wants to leave.\n\nI open my eyes.\nThe ceiling is still here.\nThe blanket has changed nothing.\nNothing.\n\nThe night continues with administrative determination.\nSomewhere, a bird begins singing.\nI am offended by its optimism.\n\nThe sky is becoming lighter.\nI never actually fell asleep.\nI only spent seven hours thinking about it.\n\nTomorrow, I will try again.\nAnd tonight's questions will be waiting for me exactly where I left them.\nWaiting.\n\n...maybe\u2026\n\u2026Tuesday"
      },
      {
        title: "The Elevator Face",
        lyrics: "[THE ANXIOUS ONE]\nThe face.\nThe elevator face.\nThe cables are vibrating.\nThe air is thinning out.\nThere is something in the hallway pretending to be normal.\nIt knows my name.\nI never gave permission.\nI am cataloguing the danger.\nSomeone has to check the oven.\nSomeone has to check the door again.\nSomeone already checked it.\nBut that was before it learned to move.\n\n[THE HYSTERIC]\nDo not mention nineteen-eighty-seven.\nIt answers when you do.\nIt answers when you don't.\nThe cables are shaking!\nThe walls are moving!\nSomething is breathing behind the door!\nWe are voting on breathing again!\nGranted.\nDenied.\nGranted.\nDenied.\nThe butterfly has seven eyes!\nIt is watching the elevator!\nThe elevator is watching back!\n\n[THE NOSTALGIC]\nWe are still standing on that sidewalk.\nWe never left it.\nWe only learned to rename it.\nI remember when breathing was spontaneous.\nI remember when silence was not suspicious.\nEverything was blue before the static started.\nThe blue shirt.\nThe cold laughter.\nThe moment that never ended.\nNineteen-eighty-seven is calling again.\nI can hear it.\nI can hear it.\nIt is always calling.\n\n[THE JUDGE]\nThe door must be checked.\nThe elevator must be stopped.\nThe past must be recorded.\nThe sequence must be corrected.\nThe butterfly knows!\nThe hallway knows!\nThe room knows!\nSomething is wrong.\nSomething is always wrong.\nWho authorized this sequence?\nWHO AUTHORIZED THIS SEQUENCE?\n\n[PROTAGONIST]\nI have been here the entire time.\nChecking.\nRecording.\nPreventing collapse.\nThirty-four years.\nPreventing something that never arrives.\nNot as a system.\nNot as a function.\nJust...\nI only came into this room for something simple.\nSomething I already forgot.\nWho was that?\nI don't know.\nIt sounded tired.\n\n[THE JUDGE]\nGranted.\nDenied.\nGranted.\nDenied.\nSTOP CHECKING THE DOOR."
      },
      {
        title: "My Beloved",
        lyrics: "My beloved left me.\nShe didn't say much.\nShe just went away.\n\nShe left me.\nShe went away.\nShe took with her all of her things.\n\nShe went away without giving explanations.\nShe doesn't want to talk anymore.\nShe doesn't answer the phone.\n\nI feel so alone.\nI need her.\nI miss her so much.\nI need my beloved.\n\nWhy did you go away?\nWhy did you slam the door?\nWhy were you so DISGUSTED?!\nSo scared and so tired?\nWhy?\n\nI remember your shocked face when you opened the drawer\nand found my shit carefully arranged next to your socks\u2026\nI REMEMBER YOUR SCREAMS!\nYour wide open eyes...\nRunning to the bathroom to go and vomit.\n\nI remember when I called the pizza place and ordered 4 pizzas.\nAnd you asked\n4 pizzas? For who?\nOne for me!\n3 for the friends who live inside my head!\nEveryone has their tastes!\n\nYou were holding your head in your hands.\nSometimes you cried\nYou walked away.\nBut then you came back.\n\nThe notes I left stuck on the walls to remind me of my name and what I had to do\u2026\nToothpaste to clean the hair!\nTrash used as furniture!\nClosed windows to keep out light and noise!\n\nROTTING VEGETABLES!\nEMPTY BOXES!\nFILLED WITH THOUGHTS!\nAND USED CONDOMS!\nAND USED CONDOMS!\n\nShe closed the door behind her.\nShe said she couldn't take it anymore.\nShe was screaming\ncrying\nVOMITING\nShe said I was insane.\nA disgusting fucking insane man.\n\nWhy my beloved did you go away?\nYour dirty underwear.\nI miss it so much.\nI need you\nI NEED YOU!\n\nAnd you my beloved you need me.\nBut when finally\nTUESDAY HAS ARRIVED\nall of this will end\nand you will come back to me!\n\nThat day even you will keep your shit inside the drawers!\n\nWHEN TUESDAY COMES YOU WILL UNDERSTAND!\nWHEN TUESDAY COMES YOU WILL COME BACK!"
      },
      {
        title: "Still Here",
        lyrics: "(Hey bro, are you home?)\nYes guys, I'm here...\n(Can you open the door?)\nSorry boys, I can't!\n(Hey bro, why can't you?)\nI'm negotiating with the lamp.\n(Sorry bro... what?!)\nIt's complicated.\nWe've been talking for most of the morning.\nThe lamp wants changes!\nI'm trying to be reasonable.\n\n(Please come out. We miss you.)\nI miss you too.\n(Then open the door.)\nI can't leave the apartment unsupervised!\n(It's an apartment. It doesn't need supervision.)\nYou don't know this apartment!\n\nThe bedroom still isn't speaking to the hallway.\nThings are calmer than last week, but I don't want to risk a setback.\nSorry not today!\nSorry not today!\nSORRY NOT TODAY!\n\n(What did you eat today?)\nI had a conversation with the refrigerator!\nIt was very productive!\nWe reached an agreement about several ongoing issues!\n(That's not eating.)\n\n(Come on. Just for an hour. Fresh air.)\nI can't.\nThe corridor has become hostile.\n(The corridor? You're talking about the corridor?)\nI DON'T MAKE THE RULES!\nIt's been getting worse since last week.\nI'm sorry, guys.\nBut not today!\n(Why not today?)\nSORRY NOT TODAY!\nSORRY NOT TODAY!\nSORRY NOT TODAY!\n\nI'm cataloguing the shadows.\nThere are more than yesterday.\nSome of them weren't here before.\nSOMEONE HAS TO KEEP TRACK!\n\n(We're worried about you.)\nDon't be.\nEverything is under control.\n(Are you sure about that?)\nMostly.\nThe ceiling and I have an understanding.\nThe washing machine is nervous but stable!\nThe refrigerator has become cooperative.\nProgress is being made.\n\n...listen\u2026\nDon't tell the curtains\u2026\nThey've started moving without wind..\nI don't know what they want yet\u2026\nBut I'm watching them.\n\n(Come on man, let's go for a walk. Let's grab a beer.)\nI CAN'T!\nI'M IN THE MIDDLE OF A VERY DELICATE PHASE!\n\n(Ehy Bro... c'mon open the door.)\nSorry guys, I can't.\nThe trash needs emotional support.\n(The trash... what do you mean?)\nIt's carrying a lot right now.\nPeople underestimate that.\n(We could help...)\nYOU CAN'T COME IN!\nThe ecosystem is calibrated.\nOne additional person would destabilize everything.\n(What ecosystem?)\nThis one.\nThe apartment has finally stopped arguing with itself.\nI'd like to see how long that lasts!\n\n(Come on man... open the door.)\nNot today, guys.\n(Why not today?)\nSORRY NOT TODAY!\nSORRY NOT TODAY!\nSORRY NOT TODAY!\n\n(Tomorrow then?)\nNot tomorrow either.\nBut soon.\nReally soon.\n\n(We love you, you know.)\nI know.\n(So come on, open the door.)\nI can't.\nThe lamp is still waiting.\nWe've made progress, but there are unresolved issues.\n\n(We'll be back tomorrow.)\nI know.\nThank you guys.\n\n(Hey bro, still here?)\nStill here.\n(Come with us. Just this once.)\nI promise boys.\nI promiiise.\nI'll come with you.\nTUESDAY!\n\nSORRY NOT TODAY!\nSORRY NOT TODAY!\nSORRY NOT TODAY!\nSORRY NOT TODAY!\n\nI promise\nTUESDAY."
      },
      {
        title: "Church Of Incoming Tuesday",
        lyrics: "I have been a member for three years.\nOr maybe four.\nThe Church does not count days.\nOnly Tuesdays.\n\nThe Church does not recruit.\nThe Church is a pre-existing condition.\nI have attended every service.\n\nThe doctrine is clear.\nTuesday is coming.\n\nEvery object has a soul.\nThis is doctrine.\nThe trash is conscious.\nThis is doctrine.\nChairs become lonely.\nThis is doctrine.\nUnopened mail develops opinions.\nThis is doctrine.\n\nI am not waiting.\nI am preparing.\nI am not malfunctioning.\n\nI am the only baseline specimen in a crooked grid.\nThe rest of the world is a clerical error walking sideways.\nBut the margins are closing.\n\nWhen Tuesday comes I will sleep.\nWhen Tuesday comes she will understand.\nWhen Tuesday comes the others will see.\nWhen Tuesday comes every procedure will make sense.\n\nThe forms will stop arguing.\nThe instructions will stop contradicting themselves.\n\nThe Church teaches I am not stuck.\nI am early.\nThe Church teaches I am not hiding.\nI am correctly positioned.\n\nThe world will not be redeemed.\nThe world will finally apologize.\nThe schedule remains intact.\nNothing has been postponed.\n\nI am ready.\nThe Church is ready.\nTuesday is coming.\nTuesday is coming.\nTUESDAY IS COMING!\n\n(THE DAY APPROACHES!)\nTHE DAY APPROACHES!\nTHE DAY APPROACHES!\n(HAVE YOU SEEN THE SIGNS?)\nWE HAVE SEEN THE SIGNS!\nWE HAVE SEEN THE SIGNS!\n(TRUST THE SIGNS!)\nTRUST THE SIGNS!\nTRUST THE SIGNS!\n(THE DELAY IS SACRED!)\nTHE DELAY IS SACRED!\nTHE DELAY IS SACRED!\n(ANOTHER TUESDAY CLOSER!)\nANOTHER TUESDAY CLOSER!\nANOTHER TUESDAY CLOSER!\n(HAVE YOU PREPARED?)\nWE HAVE PREPARED!\nWE HAVE PREPARED!\n(PREPARATION IS SALVATION!)\nPREPARATION IS SALVATION!\nPREPARATION IS SALVATION!\n(THE GREAT AUDITOR NEVER BLINKS!)\nTHE GREAT AUDITOR NEVER BLINKS!\nTHE GREAT AUDITOR NEVER BLINKS!\n(THE TRASH IS CONSCIOUS!)\nTHE TRASH IS CONSCIOUS!\nTHE TRASH IS CONSCIOUS!\n(THE RECEIPTS HAVE BEEN FORGIVEN!)\nTHE RECEIPTS HAVE BEEN FORGIVEN!\nTHE RECEIPTS HAVE BEEN FORGIVEN!\n(THE FURNITURE IS COOPERATING!)\nTHE FURNITURE IS COOPERATING!\nTHE FURNITURE IS COOPERATING!\n(WHO WILL CHANGE?)\nNOT US!\n(WHO WILL CHANGE?)\nTHE OTHERS!\n(WHO WILL CHANGE?)\nEVERYTHING ELSE!\n(THE WORLD IS ALIGNING!)\nTHE WORLD WILL ALIGN!\nTHE WORLD WILL ALIGN!\n(WILL SHE UNDERSTAND?)\nSHE WILL ALIGN HER SOCKS IN PROPER CHRONOLOGY!\n(THE PARKING METERS HAVE ACCEPTED TUESDAY!)\nTHE PARKING METERS HAVE ACCEPTED TUESDAY!\nTHE PARKING METERS HAVE ACCEPTED TUESDAY!\n(WE ARE NOT CRAZY!)\nWE ARE NOT CRAZY!\n(WE ARE...)\nWE ARE EARLY!\nWE ARE EARLY!\n(THE DAY APPROACHES!)\nANOTHER TUESDAY CLOSER!\nANOTHER TUESDAY CLOSER!\nTUESDAY!\nTUESDAY!\nTUESDAY!"
      },
      {
        title: "The Church Choir",
        lyrics: "[Reprise \u2014 gospel intermezzo, reprising the closing section of \"Church Of Incoming Tuesday\"]\n\n(THE DAY APPROACHES!)\nTHE DAY APPROACHES!\nTHE DAY APPROACHES!\n(HAVE YOU SEEN THE SIGNS?)\nWE HAVE SEEN THE SIGNS!\nWE HAVE SEEN THE SIGNS!\n(TRUST THE SIGNS!)\nTRUST THE SIGNS!\nTRUST THE SIGNS!\n(THE DELAY IS SACRED!)\nTHE DELAY IS SACRED!\nTHE DELAY IS SACRED!\n(ANOTHER TUESDAY CLOSER!)\nANOTHER TUESDAY CLOSER!\nANOTHER TUESDAY CLOSER!\n(HAVE YOU PREPARED?)\nWE HAVE PREPARED!\nWE HAVE PREPARED!\n(PREPARATION IS SALVATION!)\nPREPARATION IS SALVATION!\nPREPARATION IS SALVATION!\n(THE GREAT AUDITOR NEVER BLINKS!)\nTHE GREAT AUDITOR NEVER BLINKS!\nTHE GREAT AUDITOR NEVER BLINKS!\n(THE TRASH IS CONSCIOUS!)\nTHE TRASH IS CONSCIOUS!\nTHE TRASH IS CONSCIOUS!\n(THE RECEIPTS HAVE BEEN FORGIVEN!)\nTHE RECEIPTS HAVE BEEN FORGIVEN!\nTHE RECEIPTS HAVE BEEN FORGIVEN!\n(THE FURNITURE IS COOPERATING!)\nTHE FURNITURE IS COOPERATING!\nTHE FURNITURE IS COOPERATING!\n(WHO WILL CHANGE?)\nNOT US!\n(WHO WILL CHANGE?)\nTHE OTHERS!\n(WHO WILL CHANGE?)\nEVERYTHING ELSE!\n(THE WORLD IS ALIGNING!)\nTHE WORLD WILL ALIGN!\nTHE WORLD WILL ALIGN!\n(WILL SHE UNDERSTAND?)\nSHE WILL ALIGN HER SOCKS IN PROPER CHRONOLOGY!\n(THE PARKING METERS HAVE ACCEPTED TUESDAY!)\nTHE PARKING METERS HAVE ACCEPTED TUESDAY!\nTHE PARKING METERS HAVE ACCEPTED TUESDAY!\n(WE ARE NOT CRAZY!)\nWE ARE NOT CRAZY!\n(WE ARE...)\nWE ARE EARLY!\nWE ARE EARLY!\n(THE DAY APPROACHES!)\nANOTHER TUESDAY CLOSER!\nANOTHER TUESDAY CLOSER!\nTUESDAY!\nTUESDAY!\nTUESDAY!"
      },
      {
        title: "The System Loses Control",
        lyrics: "I had a plan.\nI wrote it down.\nThen I lost the paper.\nThen I found the paper.\nThe paper disagreed with the plan!\n\nThe toaster has been staring at me all morning.\nI don't trust that level of commitment.\n\nI should take out the trash.\n(The trash should be preserved. It contains evidence.)\nRight.\nOf course.\n\nI walked into the kitchen.\nI forgot why.\nThe kitchen remembered.\nI didn't ask.\n\nI think I need more sleep.\n(I haven't slept since 1984.)\nThat's impossible.\n(So is Tuesday.)\nFair point.\n\nThe cucumber informed me that it missed its shift.\nI didn't know cucumbers worked.\n(\"Some people use cucumbers for unrelated activities...\")\nPLEASE, STOP HELPING!\n\nMy left shoe is making accusations.\nMy right shoe refuses to comment.\nThe investigation continues.\n\nGod, I miss my beloved.\n(Not as much as you miss yourself, ASSHOLE!)\nThank you.\nThat was unnecessary.\n(It was extremely necessary.)\n\nCONGRATULATIONS!\nCONGRATULATIONS!\nCONGRATULATIONS!\n\nSomething is wrong with the walls.\nThe walls are too wall today.\n\nI found a receipt in my pocket.\nIt was older than I am.\n\nThe refrigerator made a noise.\nEveryone froze.\nNobody wanted to be the first to interpret it.\n\nWait.\nWait.\nWait.\n...\nOh fuck...\nI put the SHIT in the trash...\n...\nOh wait.\nWait!\nWAIT!!!\n\nOH SHIT!!!\nI PUT THE SHIT IN THE TRASH!!!\n\nLadies and gentlemen!\nA brief administrative emergency!\nThe shit has been misplaced!\nThe cucumber demands representation!\nThe refrigerator refuses to comment!\nNobody remains qualified!\n\nCONGRATULATIONS!\nCONGRATULATIONS!\nThank you for your cooperation!\nThank you for your cooperation!\nThank you for your cooperation!\n\n(Maybe the procedure is compromised.)\n(i tell you...The procedure was always compromised.)\n(I liked it better when the shit had purpose.)\n(We can still recover it.)\n(We absolutely cannot recover it.)\nNobody agrees!\nNobody ever agrees!\n\nThe cucumber has returned.\nIt wants a supervisor.\n\nMy beloved is leaving again.\nI don't remember inviting her.\n(You never do.)\n\nI should make a list.\n(No.)\nI need a list.\n(No.)\nI am becoming difficult to organize.\n(That happened years ago.)\n\nThe room is rotating slightly.\nOr maybe I am.\nOr maybe the room is rotating me.\n\nI think Tuesday is getting closer.\n(I think Tuesday is avoiding us.)\n(That's not doctrine.)\n(Nothing is doctrine anymore.)\n\nExcuse me.\nWho said that?\nWhy are there so many of us?\nWho's talking?\nWhich one is me?\n(Which one was you?)\n\nWait.\nWho said that?\n(All of us.)\nNo.\nThat's not\u2014\nNo.\n\n(Check the list.)\nI don't have the list.\n(Exactly.)\nWhich one is me?\n(The cucumber knows.)\nWhat?\n(Ask the cucumber.)\nI don't trust the cucumber.\n(The cucumber doesn't trust you either.)\nThat's ridiculous.\n(Says the man who lost the shit.)\nI didn't lose it.\n(You threw it away.)\n(You abandoned it.)\n(You violated procedure.)\n(You forgot the list.)\n(You forgot her.)\n(You forgot yourself.)\n\nStop.\n(Which one of us said stop?)\nStop.\n(Not authorized.)\nSTOP.\n\n(Please hold.)\n(Your identity is being reviewed.)\nWhich one is me?\nOh, SHIT!!!"
      },
      {
        title: "Tropical Serenity Oasis",
        lyrics: "Welcome to the Tropical Serenity Oasis...\nI wasn't expecting the upgrade...\nBut checking in is mandatory...\nAnd the sun never...\never...\nEVER...\nsets...\n\nTwo very enthusiastic receptionists arrived at my door!\nThey didn't knock!\nThey removed the entire front door!\nVery proactive!\nMost hotels would have sent an email!\n\nThey carried me all the way to the shuttle!\nI didn't even have to pack!\nFive stars already!\n\nBlue lights on the roof...\nVery trendy...\nVery clubbing...\nThe siren...\nWoooo! woooo! woooo!\nBeautiful rhythm...\nIt clears the traffic just for me...\nThey treat me like a head of state...\n\nThe resort doctor greeted me personally...\nHe offered me the welcome drink...\nA small pink tablet...\nDissolved in mineral water...\nHints of strawberry...\nHints of chalk...\nLuxury...\nPure luxury...\n\nThe tailor was already waiting...\nNo sleeves...\nMinimalist design...\nScandinavian elegance...\nIt fastens firmly behind my back...\nExcellent posture support...\nI can barely move...\nThat's what luxury feels like...\nTHAT is luxury!\n\nThe room is extraordinary...\nSoft walls...\nSoft floor...\nSoft silence...\nFinally...\nA room that doesn't argue with itself...\n\nThe windows don't open...\nKeeps the tropical climate inside...\nThe door locks automatically...\nExclusive...\nImpeccable service...\n\nThere was a cucumber in the corridor...\nNew uniform...\nPromotion...\nGood for him...\n\nThe refrigerator called later...\nIt finally received the apology it deserved...\nI'm genuinely happy for both of them...\n\nI think I saw my beloved near the swimming pool...\nWhite lab coat...\nClipboard...\nVery professional...\nShe must work here...\nGood for her...\n\nEverything is under control...\nEverything is being reviewed...\n\nFor a moment...\nThe blue lights looked different...\nThe uniforms looked different...\nThe room looked different...\nNot worse...\nJust...\ndifferent...\n\nThen the third welcome drink arrived...\nMuch better...\n\nI asked the concierge:\nIs tomorrow Tuesday?!\n(Here it is always summer, sir!)\nWHAT?!\nWhat time is checkout?!\n(Never!)\nEXCELLENT!\nDo you have Tuesday on the menu?!\n(Everything is included!)\nAH! AH!\n\nWHO NEEDS TUESDAY?!\nWHO NEEDS APPOINTMENTS?!\nWHO NEEDS RESPONSIBILITIES?!\nTHIS PLACE IS ALL-INCLUSIVE!\nEVERYTHING IS INCLUDED!\nANOTHER WELCOME DRINK!\nANOTHER ONE!\n\nTUESDAY IS FOR THE WORKERS!\nTUESDAY IS FOR THE PLANNERS!\nTUESDAY IS FOR THE KITCHEN ORGANIZERS!\nTUESDAY IS FOR THE PEOPLE WHO REMEMBER WHAT DAY IT IS!\nTUESDAY IS FOR THE PEOPLE WHO STILL HAVE APPOINTMENTS!\n\nThis is paradise!\nThis is service!\nThis is EVERYTHING!\n\nExcuse me...\nDo you have Tuesday on the menu tomorrow?\n(You already asked.)\nDid I?\n(Everything is included.)\nExcellent...\nThis place...\nhas everything...\n\nTropical...\nSerenity...\nOasis...\nTropical...\nSerenity\u2026.\nOasis..."
      }
    ]
  },
  // Per il prossimo album: copia il blocco qui sopra (da { a },)
  // e incollalo qui sotto con i nuovi contenuti.

];
